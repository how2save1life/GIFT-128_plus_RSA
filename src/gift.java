...
class gift {
    ...
    String giftEncrypt(String Plain, String Key) {
        ...
        for (int i = 0; i < 48; i++) {
            String s = getSubCells(plain);
            System.out.println(i + ":   SubCells: " + s);
            String p = getPermBits(s);
     ...
     */
    //s盒替换
    private String getSubCells(String str) {
        str = str.toUpperCase();
        ...
        for (int i = 0; i < str.length(); i++) {//对每个16进制数
            ...
            for (String key : sBox.keySet()) {//如果该16进制数符合s盒
                ...
                if (str.charAt(i) == key.charAt(0)) {//如果找到
                    ...
                    result.append(value);//result[i]的值
                    break;
                }
            }
        }
     ...
     */
    //p盒替换//127...0
    private String getPermBits(String hexStr) {
